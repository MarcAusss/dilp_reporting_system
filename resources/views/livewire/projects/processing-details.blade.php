                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($project->procurements->sortByDesc('id') as $record)
                                <tr>
                                    ...
                                    <td class="px-5 py-4 text-xs text-slate-600"><p>{{ $record->supplier ?: '—' }}</p><p class="mt-1 text-slate-400">{{ $record->procurement_date?->format('M d, Y') ?? '—' }}</p></td>
    ...
    @if ($activeSection === 'replacement')
        <div class="space-y-4">
            ...
            @if ($showReplacementForm)
                ...
                </div><div><label class="block text-sm font-medium text-slate-700">Reason *</label><textarea rows="3" wire:model="replacement_reason" class="mt-2 block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm"></textarea>@error('replacement_reason')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div><div><label class="block text-sm font-medium text-slate-700">Remarks</label><textarea rows="3" wire:model="replacement_remarks" class="mt-2 block w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm"></textarea></div><div class="flex justify-end gap-2 border-t border-slate-100 pt-5"><x-ui.button type="button" variant="secondary" wire:click="cancelReplacement">Cancel</x-ui.button><x-ui.button type="submit">Save Replacement Request</x-ui.button></div></form></x-ui.card>
            @endif
